...
if 'data_df' not in st.session_state or st.session_state['data_df'] is None:
    ...
    # ==================================================================
        ...
        if "Bowler Name" in df_raw.columns:
            df_bowler_totals = df_role_base.groupby("Bowler Name").agg(
    ...
    # ==================================================================
        ...
        with col3:
            if f2 in ["Economy By Length", "% by Lengths"]:
                ...
                # Layout setup based on notebook placeholder logic for tracking Turn angles
                f3 = st.selectbox("Select Ball Break Direction", ["Away from Batter", "Into Batter", "No Turn"])
                filter_label = f3
                
                # Placeholder connection: currently utilizes full spinner base until you declare the spin deviation column name
        ...
        if "Bowler Name" in df_raw.columns:
            ...
            if not df_filtered.empty:
                ...
                if not leaderboard.empty:
                    ...
                    # Manage display formatting states
                    if f2 in ["% by Lengths", "% /Away/No/In (TURN)"]:
                        ...
                        final_cols = ["Bowler Name", "Balls_Bowled", "Wickets", "Economy", "% Metric"]
